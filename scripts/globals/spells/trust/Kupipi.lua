...
function onMagicCastingCheck(caster,target,spell)
	...
	elseif (bf ~= nil) then
		...
		elseif (not caster:isUniqueAlly(898)) then
...
function onSpellCast(caster,target,spell)
	caster:spawnAlly(898);
	caster:PrintToPlayer(string.format("(Kupipi) The professor always said that books make the best weapon.", caster:getName()),0xF);    
	return 0;
end;